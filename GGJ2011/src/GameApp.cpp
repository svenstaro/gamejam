...
void GameApp::Init(char* argv[]) {
	...
	if(mFullscreen) {
        ...
        mRenderWin->Create(sf::VideoMode(WIDTH_WINDOW, HEIGHT_WINDOW, 32), "AI and the bomb", sf::Style::Default, sf::ContextSettings(24, 8, 4));
        mRenderWin->SetPosition(WIDTH_WINDOW / 2 - mRenderWin->GetWidth() / 2,sf::VideoMode::GetDesktopMode().Height / 2 - HEIGHT_WINDOW / 2);
    }
    ...
    if(mFullscreen) {
        // TODO: black borders
        mView->SetViewport(sf::FloatRect(0, (76.0 / HEIGHT_WINDOW), 1.0, 1.0 - (76.0 / HEIGHT_WINDOW)));
        //mView->SetViewport(sf::FloatRect(0, 0, 1.0, 1.0));
    } else {
        mView->SetViewport(sf::FloatRect(0, (76.0 / HEIGHT_WINDOW), 1.0, 1.0 - (76.0 / HEIGHT_WINDOW)));
    }
    mView->SetSize(WIDTH_WINDOW, 624);
    mView->SetCenter(WIDTH_WINDOW / 2, 624 / 2);

    //mView = boost::shared_ptr<sf::View>(new sf::View());
    //mView->SetViewport(sf::FloatRect(0, 0, 1.0, 1.0));
    //mView->SetSize(mRenderWin->GetWidth(), mRenderWin->GetHeight());
    //mView->SetCenter(0, 0);
    //mView->SetCenter(mRenderWin->GetWidth() / 2, mRenderWin->GetHeight() / 2);
        ...
        if(IsEditorMode()) {
            ...
            if(mAppMode == AM_PUZZLE) {
                /*sf::Text t("Place a Mover by clicking on the rail.");
                  ...
                  t.SetPosition(floor(mRenderWin->GetWidth() / 2 - t.GetRect().Width / 2), 80);
                ...
                t.SetPosition(mRenderWin->GetWidth() / 2 - 70, 20);
                ...
                t.SetFont(mResourceManager.GetFont("lcd"));
                t.SetPosition(mRenderWin->GetWidth()/2 + 43, 32);
                t.SetColor(sf::Color(150,150,150));
                mRenderWin->Draw(t);
...
void GameApp::ToggleFullscreen() {
    ...
    if(mFullscreen) {
        ...
        mRenderWin->Create(sf::VideoMode(WIDTH_WINDOW, HEIGHT_WINDOW, 32), "AI and the bomb", sf::Style::Default, sf::ContextSettings(24, 8, 4));
        mRenderWin->SetPosition(sf::VideoMode::GetDesktopMode().Width / 2 - WIDTH_WINDOW / 2,sf::VideoMode::GetDesktopMode().Height / 2 - HEIGHT_WINDOW / 2);
    }
    mRenderWin->EnableVerticalSync(true);
    mRenderWin->ShowMouseCursor(false);
}
